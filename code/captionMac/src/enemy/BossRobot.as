...
package enemy
{
	...
	public class BossRobot extends Enemy
	{
		
		[Embed(source = '../img/boss/bossrobot.png')] public   var brobotpng:Class;
		[Embed(source = '../img/sailor/boss6dead.png')] public   var bdpng:Class;
		...
		public function BossRobot(X:Number,Y:Number,Type:int)
		{
			x=X
			y=Y	
			health=maxlife=3000
			...
			if(type==0){	
				this.loadGraphic(brobotpng,true,true,150,200,false)
				this.addAnimation(Actions.STATE_CATCH,[32],0,false)	
				this.addAnimation(Actions.IDLE,[32],0,false)	
				...
				this.addAnimation(Actions.WALK,[32,31,30,29,28,27,26,25,24,23,22,21,20,19,18,17],10,true)
				
				//this.addAnimation(Actions.DAMAGE_CATCH,[7,6,5,4],10,false)
				this.addAnimation(Actions.DAMAGE_NORMAL,[5,4,3],6,false)
				...
				this.addAnimation(Actions.DROPDOWN_FACE,[1,1,1,1,1,0,0,0,0],8,false)	
				...
				this.addAnimation(Actions.ATTACK_NORMAL,[13,12,11,10],8,false)		
				this.addAnimation(Actions.ATTACK_RUN,[16,15,14,13,12,11,10,9,8,8,8,8],10,false)
				this.addAnimation(Actions.ATTACK_RUNJUMP,[6,5,4,3,2],8,false)
			}
		...
		public function chancestate():void
		{
			...
			if(currentstate==Actions.ATTACK_RUN && !finished){
				if(player1.currentstate!=Actions.DROPDOWN_FACE && FlxU.abs(y+height-player1.y-player1.height)<50 && FlxCollision.pixelPerfectCheck(this as FlxSprite,player1 as FlxSprite) ){
					States.setDamagePlayerState(Actions.DROPDOWN_FACE,player1,20)
				}	
			}
			
//			if(currentstate==Actions.ATTACK_RUNJUMP && finished)
//			{
//				if(player1.currentstate!=Actions.DROPDOWN_FACE && player1.acceleration.y==0 ){
//					States.setDamagePlayerState(Actions.DROPDOWN_FACE,player1,20)
//				}
//			}
		...
		override public function update():void
		{
			...
			if( this.acceleration.y>0 && velocity.y>0 &&  y>=starty )
			{
				...
				if(currentstate!=Actions.DROPDOWN_FACE && Registry.player.currentstate!=Actions.DROPDOWN_FACE && Registry.player.acceleration.y==0 ){
				...
				if( isinterrupt()){
					...
					if(findtarget) //发现玩家
					{
						...
						if(rndmove==0)
						{
							...
							var rndstate:int=Math.random()*3
							//var rndstate:int=0
							...
							else if(rndstate==2 && FlxU.abs(this.y+this.height-player.y-player.height)<50 && FlxU.abs(this.x+this.width-player.x-player.width)<200){
								FlxG.play(Registry.mmattack1mp3,10,false);
			...
			if(health<=0 )
			{
				...
				if(this.exists)
				{
					var tmped:Expolre=new Expolre(x,y,4)
					//X:Number,Y:Number,type:int=0,face:uint=FlxObject.RIGHT)
					Registry.fxgroup.add(tmped)
				}
