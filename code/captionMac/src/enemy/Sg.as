...
package enemy
{
	...
	public class Sg extends Enemy
	{
		
		[Embed(source = '../img/boss/sg.png')] public   var sgpng:Class;
		//	[Embed(source = '../img/weapon/gunbullet.png')] public   var gunbulletpng:Class;
		[Embed(source = '../img/sailor/sgdead.png')] public static  var sgdead1png:Class;
		private var delayAI:FlxDelay
		private var findtarget:Boolean=false
		//public var type:int
		//	public var shoot:FlxWeapon
		private var sy:Number
		public function Sg(X:Number,Y:Number,Type:int)
		{
			x=X
			y=Y	
			health=80
			...
			starty=y	
			if(type==0){	
				this.loadGraphic(sgpng,true,true,100,100,false)
				this.addAnimation(Actions.STATE_CATCH,[13],0,false)	
				this.addAnimation(Actions.IDLE,[10],0,false)	
				...
				this.addAnimation(Actions.WALK,[15,16],10,true)
				
				this.addAnimation(Actions.DAMAGE_CATCH,[17,18,19],10,false)
				this.addAnimation(Actions.DAMAGE_NORMAL,[17,18,19],10,false)
				this.addAnimation(Actions.DAMAGE_THROWOUT,[1,1,0,0,8,9],8,false)
				//this.addAnimation(Actions.DROPDOWN_BACKFACE,[27,28,29,30,31,32,33],8,false)	
				this.addAnimation(Actions.DROPDOWN_FACE,[1,1,0,0,8,9],8,false)	
				...
				this.addAnimation(Actions.ATTACK_NORMAL,[7,6,5,4,3],10,false)		
		...
		override public function update():void
		{
			...
			if(health<=0 )
			{
				...
				//if(dd==0){
				f1.setBody1(sgdead1png,true,8,[0,1,2,3,4],100,100)
